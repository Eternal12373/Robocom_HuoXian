...
static uint8_t SingleCircle(void){
    ...
    switch(SingleMode){
        ...
        case 17:                                               // 根据openmv提供的位置信息，去夹取，刚刚被抛投出去的沙包 
            MoveMode=Drive;
            SetCarSpeed=150;
            if(SingleTim>3000)CarAngle=LastCarAngle+Openmv.x/400.0;
            if(CarAngle<-45)CarAngle=-45;
            if(CarAngle>45)CarAngle=45;
        ...
        case 22:            
            ...
            if(CrossDetect(Crossing)&&SingleTim>800){
                ...
                if(SingleTim>600) SingleTim=600;
            }
            break;
        case 23:     
            MoveMode=Drive;
        ...
        case 28:                                               // 根据目标的颜色，来决定跳转到哪个case分支
			if(BallColor==4) SingleMode=SingleMode+3;
			else if(BallColor==2) SingleMode=SingleMode+2;
			else if(BallColor==1) SingleMode=SingleMode+1;
            break;
        case 29:    
...
static void TurnStraightUntil(int16_t SetStraightSpeed,int16_t SetTurnAngle,uint8_t Cross)
{
    ...
    else if(TurnStraightSign==1)
    {
        ...
        if(CrossDetect(Cross)&&SingleTim>200){    // 检测到十字路口停车
            SingleTim=0;
            MoveMode=Stop;
            if(SetStraightSpeed<0) TurnStraightSign=2;        // 设定速度小于0， 进入TurnStraightSign==2分支
            else if(SetStraightSpeed>400) TurnStraightSign=3; // 设定速度大于0， 进入TurnStraightSign==3分支
            else {
				TurnStraightSign=0;
				SingleMode++;
            }
...
static void StraightUntil(int16_t SetStraightSpeed,int16_t SetAngle,uint8_t Cross)
{
    ...
    if(TurnStraightSign==0)
    {
        ...
        if(CrossDetect(Cross)&&SingleTim>150){
            SingleTim=0;
            MoveMode=Stop;
            if(SetStraightSpeed<0) TurnStraightSign=1;
            else if(SetStraightSpeed>0) TurnStraightSign=2;
            else {
				TurnStraightSign=0;
				SingleMode++;
            }
